...
List Favorites=Alt+L, Open a file from the list of favorites; on the list Control+C and Control+Shift+C both copy the files themselves, so they paste into File Explorer as well as into text, Alt+C appends the paths as text, Delete takes every selected entry off the list
Recent Files=Alt+R, Open a file from the list of those recently used; on the list Control+C and Control+Shift+C both copy the files themselves, so they paste into File Explorer as well as into text, Alt+C appends the paths as text, Delete takes every selected entry off the list
