...
Alt+F9 either inserts a new comment or edits the one the cursor is in, and which of the two happens depends on where the cursor stands -- the same principle the table wizard follows on Control+Shift+T.  The title of the box says which it is, Insert Comment or Edit Comment, because that is where a screen reader hears it without having to ask.  Standing at the very end of a comment counts as standing in it, since that is where the cursor is left right after one is inserted.  Clearing the field of an existing comment removes that comment, and the program says so; a comment cannot be left empty by accident, because an empty box on a new comment inserts nothing at all.  A closing sequence typed inside the text is made harmless before it is written, otherwise it would end the comment early and spill the rest of the remark into the visible document.

Alt+Shift+PageDown goes to the next comment and Alt+Shift+PageUp to the previous one, reading the remark and then the word comment -- content first, as everywhere else in this program.  Neither wraps around: at the last or first comment the cursor stays where it is and the program says so, because a silent jump back to the beginning would send you looking again for a remark you had already read.  Comment List, on Control+Alt+F9, shows every comment with the line it sits on, and Enter goes to the chosen one; the line number is given rather than a count, since what you are looking for is a place in the document.  With no comment at all the commands say "No comments!".  A comment inside a fenced code block is deliberately not treated as a comment: there it is example text, the preview shows it as visible text, and jumping through it would mislead.  The two jumps used to sit on Control+Shift+F9 and Alt+Shift+F9 and moved into the PageUp and PageDown family on your decision, so that travelling between elements of a document lives in one family of keys instead of next to each function it belongs to; inserting and the list stay in the F9 family, because those write to the document and open a window rather than move the cursor.  Both freed chords, Control+Shift+F9 and Alt+Shift+F9, now belong to no command at all, and so do the bare F9 and Shift+F9, which used to drive a JAWS read-to-end script and were given up on your decision. All four commands work only on .md files and not while the preview is open, inserting requires an unguarded document, and every change can be taken back with Control+Z, with nothing written to disk.
...
To query the current font and color, press Alt+Dash.  For styles, baseline, and justification, press Alt+Slash.  These two commands report what is under the cursor; they were kept because they answer a question, rather than moving the cursor somewhere it has no reason to go.
...
List Favorites=Alt+L, Open a file from the list of favorites; on the list Control+C copies the files themselves, so they paste into File Explorer as well as into text, Control+Shift+C copies just the file names, Alt+C appends the paths as text, Delete takes every selected entry off the list
Recent Files=Alt+R, Open a file from the list of those recently used; on the list Control+C copies the files themselves, so they paste into File Explorer as well as into text, Control+Shift+C copies just the file names, Alt+C appends the paths as text, Delete takes every selected entry off the list
...
Insert Comment=Alt+F9, Insert an internal comment at the cursor, or edit the comment the cursor is in; internal comments stay in the Markdown file and are dropped when converting to Word, HTML or plain text
Next Comment=Alt+Shift+PageDown, Go to the next internal comment and read it
Prior Comment=Alt+Shift+PageUp, Go to the prior internal comment and read it
Comment List=Control+Alt+F9, Show the list of internal comments and go to the one you choose
